...
## Load functions
source('../../00_source.r')
parameters <- create_prob_vector_categ_data(n, j)

simulation_results <- obtain_group_categorical_data_simulation_distance_metrics(x = parameters$x, 
                                                                                prob = parameters$prob, 
                                                                                x_factors = parameters$x_factor, 
                                                                                m = m)
replicates <- 1
while (replicates < 1000){
  simulation_results_replicate <- obtain_group_categorical_data_simulation_distance_metrics(x = parameters$x,
                                                                                            prob = parameters$prob,
                                                                                            x_factors = parameters$x_factor, 
                                                                                            m = m)
